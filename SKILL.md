...
**注意:** 使用 `.env` 文件或环境变量管理密钥更安全，config/目录已 gitignore，不会提交到 GitHub。
...
- **加密级别**: Basic Auth (HTTP Header)
- **存储位置**: `~/.openclaw/workspace/skills/fitness-personal-assistant/config//config.json`
- **文件权限**: `600` (仅所有者可读/写)
- **网络请求**: 仅发送到 `https://intervals.icu/api/v1` 和 `https://world.openfoodfacts.org`
- **无后门**: 代码开源可审计，无任何隐藏外联

### 凭证保护最佳实践

1. **不要 commit config.json** - 已在 `.gitignore` 中配置
2. **使用环境变量** (推荐):
   ```bash
   export INTERVALS_API_KEY="your_key_here"
   export INTERVALS_ATHLETE_ID="iYourID"
   ```
3. **定期检查** - 在 intervals.icu 网页端吊销旧 Key
...
5. **无硬编码凭证**: v3.4.1+ 已移除开发者硬编码信息，从配置动态读取
